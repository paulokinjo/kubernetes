...
echo "[TASK 2] Creating a Cluster Configuration file"
...
echo "[TASK 4] Change the address of the localAPIEndpoint.advertiseAddress to the Control Plane Node's IP address"
sed -i 's/  advertiseAddress: 1.2.3.4/  advertiseAddress: 100.98.26.210/' ClusterConfiguration.yaml

echo "[TASK 5] Set the CRI Socket to point to containerd"
sed -i 's/  criSocket: \/var\/run\/dockershim\.sock/  criSocket: \/run\/containerd\/containerd\.sock/' ClusterConfiguration.yaml

#UPDATE: Added configuration to set the node name for the control plane node to the actual hostname
sed -i 's/  name: node/  name: master/' ClusterConfiguration.yaml
